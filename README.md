...
| **How you verify it** | by using it | a deterministic provider runs the pipeline with no network and no API key; 22 suites and 11 fixtures replay each run |
...
| Today & planning | a day with no plan gets suggestions; a full day gets a rest suggestion; a quarter of the day stays free, slipped flexible work moves forward, and a blocked commitment is never scheduled | `services/today.ts`, `services/workMigration.ts`, `services/dependencies.ts` |
...
as the model-backed ones, so 22 test suites and 11 acceptance fixtures replay
with no key.
...
pnpm test     # 22 suites — deterministic, offline, no API key

...
| **How you verify it** | by using it | a deterministic provider runs the pipeline with no network and no API key; 27 suites and 11 fixtures replay each run |
...
as the model-backed ones, so 27 test suites and 11 acceptance fixtures replay
with no key.
...
pnpm test     # 27 suites — deterministic, offline, no API key

...
| **怎么验证** | 用着看 | 确定性 provider 无网络、无 key 跑完整管线；27 个套件与 11 个 fixture 每次重放 |
...
流水线可离线运行：确定性 provider 与模型实现共享同一接口，因此 27 个测试套件与 11 个验收 fixture 无需任何 key 即可重放。
...
pnpm verify   # 27 个套件 + 11 个 fixture + 构建 + 校验，全部离线
pnpm demo     # 上面的捕获演示
